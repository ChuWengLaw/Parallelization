...
public:
	long count;
	// Changed from pointer double to vector double
	std::vector<double> tv;
	// Changed from pointer long to vector long
	std::vector<long> ti;
	...
	Bacteria(char* filename)
	{
		...
		// Removed count and pointer t (not needed anymore) 
		...
		for (long i = 0; i < M; i++)
		{
			...
			{
				// Insert index at end of vector ti
				ti.push_back(i);
				// Insert T-score at end of vector tv
				tv.push_back((vector[i] - stochastic) / stochastic);
			}
			// Removed the line where it sets the T-score to 0 in else statement
		}
...
double CompareBacteria(Bacteria* b1, Bacteria* b2)
{
	...
	for (int i = p1; i < b1->count; i++)
	{
		double t1 = b1->tv[i];
		vector_len1 += (t1 * t1);
	}

	for (int i = p2; i < b2->count; i++)
	{
		double t2 = b2->tv[i];
		vector_len2 += (t2 * t2);
	}
...
void CompareAllBacteria()
{
	...
	// Fixed size array of N(N-1)/2
	double* correlation = new double[number_bacteria * (number_bacteria - 1) / 2];

	// Row major conversion
	#define correlation(i,j) correlation[number_bacteria * i + j - (i + 1)*(i + 2) / 2]

	#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < number_bacteria - 1; i++)
		for (int j = i + 1; j < number_bacteria; j++)
		{
			correlation(i, j) = CompareBacteria(b[i], b[j]);
		}
